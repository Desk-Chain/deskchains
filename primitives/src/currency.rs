		...
		fn generate_token_resources() {
			...
			let mut lp_tokens = vec![
				Token {
					...
					address: EvmAddress::try_from(CurrencyId::DexShare(DexShare::Token(DESK), DexShare::Token(DUSD))).unwrap(),
				},
				Token {
					symbol: "LP_DOT_AUSD".to_string(),
					address: EvmAddress::try_from(CurrencyId::DexShare(DexShare::Token(DOT), DexShare::Token(DUSD))).unwrap(),
				},
				Token {
					symbol: "LP_LDOT_AUSD".to_string(),
					address: EvmAddress::try_from(CurrencyId::DexShare(DexShare::Token(LDOT), DexShare::Token(DUSD))).unwrap(),
				},
				Token {
					symbol: "LP_RENBTC_AUSD".to_string(),
					address: EvmAddress::try_from(CurrencyId::DexShare(DexShare::Token(RENBTC), DexShare::Token(DUSD))).unwrap(),
				},
				Token {
...
create_currency_id! {
	...
	// 0 - 19: Deskchains & Polkadot native tokens
	...
	pub enum TokenSymbol {
		// 0 - 19: Deskchains & Polkadot native tokens
		DESK("Deskchains", 12) = 0,
		DUSD("Deskchains Dollar", 12) = 1,
		DOT("Polkadot", 10) = 2,
