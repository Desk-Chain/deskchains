...
fn trading_pair_works() {
	let aca = CurrencyId::Token(TokenSymbol::DESK);
	let ausd = CurrencyId::Token(TokenSymbol::DUSD);
	let erc20 = CurrencyId::Erc20(EvmAddress::from_str("0x0000000000000000000000000000000000000000").unwrap());
	let aca_ausd_lp = CurrencyId::DexShare(DexShare::Token(TokenSymbol::DESK), DexShare::Token(TokenSymbol::DUSD));
...
fn currency_id_try_from_vec_u8_works() {
	assert_ok!(
		"DESK".as_bytes().to_vec().try_into(),
		CurrencyId::Token(TokenSymbol::DESK)
	);
}

#[test]
fn currency_id_into_u32_works() {
	let currency_id = DexShare::Token(TokenSymbol::DESK);
	assert_eq!(Into::<u32>::into(currency_id), 0x00);

	let currency_id = DexShare::Token(TokenSymbol::DUSD);
...
fn currency_id_try_into_evm_address_works() {
	assert_eq!(
		EvmAddress::try_from(CurrencyId::Token(TokenSymbol::DESK,)),
